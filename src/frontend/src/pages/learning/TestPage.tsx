...
export const TestPage = () => {
  ...
  return <PageSection>
    ...
    {testQuery.data?.has_test ? <>
      ...
      {!isTestVisible && testState === 'available' ? <div className="form-stack"><p>После начала тестирования будет создана активная попытка. Пока она не завершена, доступ к теории и подробностям прошлых попыток будет временно ограничен. Если вы покинете страницу теста, попытка сохранится, и вы сможете продолжить её позже. При нажатии кнопки «Вернуться к теории» попытка будет завершена с текущими ответами, а вопросы без ответа будут оценены в 0 баллов.</p><Button type="button" onClick={() => startMutation.mutate()} disabled={startMutation.isPending || !canStartAttempt}>Начать тестирование</Button></div> : null}
